...
Java_com_ndk_network_http_HttpClient_RequestGetCurl(JNIEnv *env, jobject thiz, jstring url,
                                                    jobject headers) {
    ...
    for (int i = 0; i < len; i++) {
        jstring header = (jstring) env->CallObjectMethod(headers, getMethodId, i);
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_ndk_network_http_HttpClient_RequestPostCurl(JNIEnv *env, jobject thiz, jstring url,
...
Java_com_ndk_network_http_HttpClient_DownloadGetCurl(JNIEnv *env, jobject thiz, jstring url,
                                                     jobject headers) {
    string dl_get_url = "https://cdn.mytoken.org/app_download/MT-mytoken-hk-release-3.3.4_mytoken_aligned_signed.apk";
    string filename ="sdcard/OdinProject/1.jpeg";
    download_get_request(dl_get_url, filename);
    return true;
}